...
public final class Scp
{
	   ...
	   */
	    ...
	    public static void escape() {
	        Location loc = new Location(Bukkit.getWorld(ScpslPlugin.getInternalConfig().getMiniWorld()), 164.0, 86.0, 6.0);
	        World world = Bukkit.getWorld(ScpslPlugin.getInternalConfig().getMiniWorld());
	        Collection<Entity> entities = (Collection<Entity>)world.getNearbyEntities(loc, 2.0, 2.0, 2.0);
	        
	        if (!entities.isEmpty()) {
	            for (Entity it : entities) {
	                if (it instanceof Player) {
	                    Player player = Bukkit.getPlayer(((Player)it).getDisplayName());
	                    switch ((ScpslPlugin.team.get(player.getDisplayName()))) {
	                        case "SCIENCE": 
	                            ...
	                            if (it != null) {
	                                Bukkit.dispatchCommand((CommandSender)Bukkit.getPlayer(player.getDisplayName()), "essentials loadkit " + player.getDisplayName() + " mtf");
	                            }
	                        ...
	                        case "DCLASS":
	                            ...
	                            if (it != null) {
	                                Bukkit.dispatchCommand((CommandSender)Bukkit.getPlayer(player.getDisplayName()), "essentials loadkit " + player.getDisplayName() + " chaos");
	                            }
	    ...
	    public static void nukeBoom() {
	        BoundingBox aaBB1 = new BoundingBox(225.0, 84.0, 157.0, -11.0, 51.0, 38.0);
	        BoundingBox aaBB2 = new BoundingBox(66.0, 237.0, 79.0, -15.0, 250.0, 117.0);
	        World world = Bukkit.getWorld(ScpslPlugin.getInternalConfig().getMiniWorld());
	        Collection<Entity> entities1 = (Collection<Entity>)world.getNearbyEntities(aaBB1);
	        Collection<Entity> entities2 = (Collection<Entity>)world.getNearbyEntities(aaBB2);
	        
	        for (Entity it : entities1) {
	            if (it instanceof Player) {
	                Player player = Bukkit.getPlayer(it.getName());
	                player.playSound(player.getLocation(), Sound.ENTITY_FIREWORK_ROCKET_BLAST, 1.0f, 0.0f);
	                ((Damageable)it).damage(1000.0);
	            }
	        }
	        for (Entity it : entities2) {
	            if (it instanceof Player) {
	                Player player = Bukkit.getPlayer(it.getName());
	                player.playSound(player.getLocation(), Sound.ENTITY_FIREWORK_ROCKET_BLAST, 1.0f, 0.0f);
	                ((Damageable)it).damage(1000.0);
